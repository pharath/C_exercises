...
int main( void )
{
	...
	for (count = 0; count < MAX; count++)
		printf("%d\t%f\n", *i_ptr++, *f_ptr++);  // pointer arithmetic: C automatically increments the pointer addresses by the size of one array element (which depends on the pointer's data type) 
